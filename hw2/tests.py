...
class ServerTest(unittest.TestCase):
    ...
    def test_invalid_method(self):
        """Tests if the server responds correctly to methods other than GET and POST(405) msg"""
        pass
    ...
    def test_directory_trailing_slash(self):
        """
        Return index.html from /dir/
        """
        response = requests.get(self.server + "/dir/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers["content-type"], "text/html")
        self.assertNotEqual(response.text.lower().find("a simple page for multimedia content"), -1)

    def test_not_a_directory(self):
        """Return 404 if request for /dir is sent"""
        response = requests.get(self.server + "/dir")
    ...
    def test_invalid_method(self):
        """Tests if the server responds correctly to methods other than GET and POST(405) msg"""
        response = self._manual_request("DELETE /index.html HTTP/1.1\n")
        self.assertTrue(response.startswith("HTTP/1.1 405"))

    def test_app_add_response(self):
        args = {"first": "ivan", "last": "nikolov"}
        response = requests.post(self.server + "/app-add", args)
        self.assertTrue(response.status_code, 200)
        self.assertNotEqual(response.text.lower().find("new student has been added"), -1)

    def test_app_add_missing_param(self):
        args = {"first": "ivan"}
        response = requests.post(self.server + "/app-add", args)
        self.assertEqual(response.status_code, 400)

    def test_app_invalid_method(self):
        response = requests.get(self.server + "/app-add")
